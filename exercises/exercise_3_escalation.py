...
def node_analyze(state):
    ...
    with console.status("[dim]LLM reviewing the diff...[/dim]"):
        analysis = llm.invoke([
            {"role": "system", "content": (
                "You are a senior code reviewer. Structured output. "
                "If confidence < 60%, generate 2–4 specific, context-rich questions "
                "about potential issues in the diff (reference file/section where applicable)."
            )},
...
def node_synthesize(state: ReviewState) -> dict:
    ...
    with console.status("[dim]LLM refining review with reviewer answers...[/dim]"):
        refined: PRAnalysis = llm.invoke([
            {"role": "system", "content": "Refine your code review using the reviewer's answers to produce a final analysis."},
