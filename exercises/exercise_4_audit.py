...
async def node_analyze(state):
    ...
    with console.status("[dim]LLM reviewing the diff...[/dim]"):
        a: PRAnalysis = await llm.ainvoke([
            {"role": "system", "content": "You are a senior code reviewer. Structured output. If confidence < 60%, generate specific escalation questions."},
...
async def node_human_approval(state):
    ...
    await audit(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="pending",
        reason=f"Awaiting reviewer approval (confidence {a.confidence:.0%})",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_commit(state):
    ...
    await audit(state, AuditEntry(
        ...
        decision=action,
        reason=f"Review comment {action}",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_auto_approve(state):
    ...
    await audit(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="auto",
        reason=f"High confidence ({a.confidence:.0%}) - no human review needed",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_escalate(state):
    ...
    await audit(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="pending",
        reason=f"Low confidence ({a.confidence:.0%}) - escalating to reviewer for input",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
    ...
    reviewer_id = os.environ.get("GITHUB_USER", "unknown")
    answers_summary = "; ".join(f"{q}→{a}" for q, a in answers.items()) if answers else "no answers"
    await audit(state, AuditEntry(
        agent_id=AGENT_ID,
        action="escalate",
        confidence=a.confidence,
